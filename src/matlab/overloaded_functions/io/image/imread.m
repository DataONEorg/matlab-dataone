...
% limitations under the License.
    ...
    if ( runManager.configuration.debug)
        disp('Called the imread wrapper function.');
    end
    
    % Remove wrapper imread from the Matlab path
    overloadedFunctPath = which('imread');
    [overloaded_func_path, func_name, ext] = fileparts(overloadedFunctPath);
    rmpath(overloaded_func_path);    
    
    if ( runManager.configuration.debug)
        disp('remove the path of the overloaded imread function.');  
    end
    ...
    if ( runManager.configuration.capture_file_reads )
        ...
        % Parse arguments to get the source name
        if nargin > 0
        ...
        % Handle local file (Todo: to handle url file which requires to download the file first)
        if ~url          
